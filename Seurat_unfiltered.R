...
pdf("unfiltered_figures/RNA_seq_UNFILTERED_analysis.pdf", width = 13, height = 10)
...
# FIXED: PCA heatmaps with proper titles
# Create a simple function to add title to DimHeatmap
print_dimheatmap_with_title <- function(seurat_obj, dims, title) {
  ...
  # Print with a main title
  print(heatmap_plot)
  # Add title using grid text (alternative approach)
  grid::grid.text(title, x = 0.5, y = 0.95, gp = grid::gpar(fontsize = 16, fontface = "bold"))
}

# Print heatmaps with titles
